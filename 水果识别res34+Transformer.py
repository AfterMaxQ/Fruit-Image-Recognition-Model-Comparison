...
# 检查 model 对象是否存在且有效
if 'model' not in locals() or model is None or not isinstance(model, HybridFruitModel): 
...
# 从已保存的模型文件名中解析 
current_eval_cnn_backbone = CHOSEN_CNN_BACKBONE if 'CHOSEN_CNN_BACKBONE' in locals() else "resnet34" 
...
# 选择CNN目标层
if gradcam_can_proceed_s6:
    # 对于HybridFruitModel，Grad-CAM 应用于CNN backbone的最后一层卷积
    if hasattr(model_for_gradcam_s6.cnn_backbone, 'layer4'): # ResNet系列
        target_cnn_layer_s6 = model_for_gradcam_s6.cnn_backbone.layer4[-1].conv2 
        if not hasattr(model_for_gradcam_s6.cnn_backbone.layer4[-1], 'conv2') and hasattr(model_for_gradcam_s6.cnn_backbone.layer4[-1], 'conv3'):
             target_cnn_layer_s6 = model_for_gradcam_s6.cnn_backbone.layer4[-1].conv3 
        ...
        if isinstance(last_block_group, nn.Sequential) and len(last_block_group) > 0:
            ...
            elif hasattr(last_block, 'conv'): target_cnn_layer_s6 = last_block.conv 
        ...
        for module_name, module in eval_model_s7.cnn_backbone.named_modules():
            if isinstance(module, nn.Conv2d):
                ...
                if not module_name.startswith("conv_stem") and \
                   ...
                   not module_name.endswith("conv_expand"): 
